...
struct BitTests {
    ...
    func testBIT_ZeroPage(value: UInt8, expectedFlags: ExpectedFlags) {
        ...
        // Z = 0, N == 0, V == 0
        memory[0xFFFC] = Opcodes6502.BIT_ZeroPage.rawValue
        memory[0xFFFD] = 0x55
        memory[0x55] = value
        cpu.A = 0x06
        
        cpu.runForTicks(3)
        #expect(cpu.readFlag(flag: .Z) == expectedFlags.Z)
        #expect(cpu.readFlag(flag: .N) == expectedFlags.N)
        #expect(cpu.readFlag(flag: .V) == expectedFlags.V)
    }
    ...
    func testBIT_Absolute(value: UInt8, expectedFlags: ExpectedFlags) {
        ...
        // Z = 0, N == 0, V == 0
        memory[0xFFFC] = Opcodes6502.BIT_Absolute.rawValue
        memory[0xFFFD] = 0x34
        memory[0xFFFE] = 0x12
